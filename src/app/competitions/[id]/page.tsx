  ...
  const handleAddPostByLink = async () => {
    ...
    try {
      const res = await fetch(`/api/competitions/${id}/upload-posts`, {
        ...
        body: JSON.stringify([{
          fbPostId: url,
          authorName: postAuthorName.trim() || "Unknown",
      ...
      const data = await res.json();
      if (res.ok) {
        setFetchMessage(postAuthorName.trim() ? `Post added for ${postAuthorName.trim()}.` : "Post added. Fill in the author name in the table.");
        setPostLink("");
        setPostAuthorName("");
        loadCompetition();
      } else {
    ...
    <div>
      ...
      {isAdmin && <div className="mb-4 p-3 bg-green-50 rounded-xl border border-green-200 flex items-center gap-3 flex-wrap">
        ...
        <input
          ...
          className="border border-green-300 rounded-lg px-3 py-1.5 text-sm font-medium w-44 focus:outline-none focus:ring-2 focus:ring-green-400"
          onKeyDown={(e) => e.key === "Enter" && handleAddPostByLink()}
        />
        <Button
